...
public class OCIJson2Srt_v1 {
    ...
    class ExtendedWordBuilder {
        ...
        void reset() {
            _prePunct = ""; _trueToken = ""; _postPunct = ""; beginTime = -1; endTime = -1;
        }
        void flushWord() {
        ...
        void addPrePunct(String pre_p, int begin, int end) {
            if (condForFlushOnPrePunct()) { flushWord(); }
        ...
        void addTrueToken(String tt, int begin, int end) {
            if (condForFlushOnTrueToken()) { flushWord(); }
            if (beginTime != -1 && beginTime != begin) {
                System.out.println("[W]: addTrueToken: begin = " + begin + ", beginTime = " + beginTime);
            }
		...
		try {
			...
			if (jtranscriptions.length() == 0) {
				...
				for (int i = 0; i < jtokens.length(); ++i) {
					...
					int begin = Math.round(1000 * Float.parseFloat(beginText));
					int end = Math.round(1000 * Float.parseFloat(endText));
			...
			System.out.println(String.format("We have %d words", _wordList.size()));
	...
	void createSegments() {
		...
		// part 3: split too long segments
		...
		for (int i = 0; i < segments.size(); ++i) {
			...
			if (seg.duration() <= _maxDuration &&  nbNeededLines <= _maxLinesPerSegment) {
				...
				//System.out.println("------ split ----- " + segments3.size());
				...
				for (int j = 0; j < newSegments.size(); ++j) {
					segments3.add(newSegments.get(j));
				}
	...
	void verifySegments() {
		...
		ArrayList<String> wordsFromJson = new ArrayList<String>();
		for (int i = 0; i < _wordList.size(); ++i) {
		...
		ArrayList<String> wordsFromSegments = new ArrayList<String>();
	...
	public static void main(String[] args) {
		...
		OCIJson2Srt_v1 ocijson2SrtV1 = new OCIJson2Srt_v1();
		...
		options.addOption(null, "max-c", true, "max chars per line (default is " + ocijson2SrtV1.getMaxCharsPerLine() + ")");
		options.addOption(null, "max-l", true, "max lines per segment (default is " + ocijson2SrtV1.getMaxLinesPerSegment() + ")");
		options.addOption(null, "max-d", true, "max duration per segment (default is " + ocijson2SrtV1.getMaxDuration() + ")");
		options.addOption(null, "max-p", true, "max pause in a segment (default is " + ocijson2SrtV1.getMaxPause() + ")");
		options.addOption(null, "max-d", true, "min duration for a segment (default is " + ocijson2SrtV1.getMinDuration() + ")");
		...
		try {
			...
			if (commandLine.hasOption("max-c")) {
				ocijson2SrtV1.setMaxCharsPerLine(Integer.parseInt(commandLine.getOptionValue("max-c")));
			}
			if (commandLine.hasOption("max-l")) {
				ocijson2SrtV1.setMaxLinesPerSegment(Integer.parseInt(commandLine.getOptionValue("max-l")));
			}
			if (commandLine.hasOption("max-d")) {
				ocijson2SrtV1.setMaxDuration(Integer.parseInt(commandLine.getOptionValue("max-d")));
			}
			if (commandLine.hasOption("max-p")) {
				ocijson2SrtV1.setMaxPause(Integer.parseInt(commandLine.getOptionValue("max-p")));
			}
			if (commandLine.hasOption("min-d")) {
				ocijson2SrtV1.setMinDuration(Integer.parseInt(commandLine.getOptionValue("min-d")));
			}
		...
		ocijson2SrtV1.loadJson(ociJsonFile);
			
		// create segments
		ocijson2SrtV1.createSegments();
		
		// only for debug !!
		//   - verify if the word sequence is the same between json and srt
		//   - verify timestamps
		ocijson2SrtV1.verifySegments();
		
		// write SRT
		ocijson2SrtV1.writeSRT(srtFile);
