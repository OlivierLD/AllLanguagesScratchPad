...
echo -e "------------------------------"
echo -e "SQL_RACE_2 is ${SQL_ENTITY_ID}"
echo -e "------------------------------"
echo -e "Hit [return]"
read -r RESP
#
...
# No FK
#
echo -en "Create EntityBackendMapping for 'sql_race_2' ? [Hit Return] "
read -r RESP
#
...
PAYLOAD_1="{
  ...
  \"tableName\": null,
  \"sqlSelect\": \"SELECT NAME as RACE_NAME, RACE_CLASS, RACE_DATE, (RACE_CLASS || ' - ' || NAME || ' on ' || RACE_DATE) as RACE_CONCAT FROM RACE WHERE UPPER(NAME) LIKE UPPER('%\${pattern}%')\",
  \"defaultOrderBy\": null,
  \"primaryKey\": null
}"
