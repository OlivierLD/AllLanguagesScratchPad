...
import { Fn, Loop, uniform, float, vec2, vec3, vec4, normalView, cameraPosition, positionWorld, floor, fract, abs, min, max, sin, dot, step, mix, atan, time, texture, pow } from "three/tsl";
...
export const whiteNoiseTexture3D = Fn(([x]) => {
...
export const simplexNoise = Fn(([v]) => {
    ...
    // Permutations
    const im = mod289(i);
    const p = permute(permute(permute(vec4(im.z, im.z, im.z, im.z).add(vec4(0.0, i1.z, i2.z, 1.0)))
...
export const makeFBM = (noiseFn, { octaves = 6, lacunarity = 2.0, gain = 0.5 } = {}) => Fn(([p]) => {
...
const patternFBM = Fn(([v]) => {
    const a = vec3(simplexFBM(v), simplexFBM(v.add(10)), v.z);
    const b = vec3(simplexFBM(v.add(a.mul(2).add(1))), simplexFBM(v.add(a.mul(2).sub(20))), v.z);
...
function generateGemPoints(count, seed, topPole, bottomPole, equatorBias, equatorRadius, radialVariance, yJitter) {
    ...
    for (let i = 0; i < count - 2; i++) {
        ...
        const raw = rng() * 2 - 1;
        const y = Math.sign(raw) * Math.pow(Math.abs(raw), 1 / equatorBias) * 0.85 +
            (rng() - 0.5) * yJitter;
        const r = equatorRadius *
            (1 - Math.abs(y)) *
...
exteriorMat.normalNode = Fn(() => {
    ...
    n.addAssign(vec3(noise).mul(noiseStrengthUniform).sub(noiseStrengthUniform.mul(0.5)));
    return vec4(n, 1);
})();
exteriorMat.colorNode = Fn(() => {
    ...
    const hue = fract(angle
        .div(Math.PI * 2)
        .add(noise.mul(noiseStrengthUniform))
        .add(0.5)
        .add(nt));
    const tau = Math.PI * 2;
    const rainbow = vec3(sin(hue.mul(tau)).mul(0.5).add(0.5), sin(hue.mul(tau).add(2.094)).mul(0.5).add(0.5), sin(hue.mul(tau).add(4.189)).mul(0.5).add(0.5));
...
interiorMat.colorNode = Fn(() => {
    ...
    Loop({ start: 0, end: interiorStepsUniform }, () => {
        ...
        const hue = fract(atan(pos.z, n)
            .div(Math.PI * 2)
            .add(pos.y.mul(0.3))
            .add(0.5)
            .add(nt));
        const tau = Math.PI * 2;
        const col = vec3(sin(hue.mul(tau)).mul(0.5).add(0.5), sin(hue.mul(tau).add(2.094)).mul(0.5).add(0.5), sin(hue.mul(tau).add(4.189)).mul(0.5).add(0.5));
        // Accumulate: noise acts as density
        // const ss = smoothstep(.9, 0.5, n).add(.1);
        accumulated.addAssign(col
...
function buildGem() {
    const pts = generateGemPoints(options.points, options.seed, options.topPole, options.bottomPole, options.equatorBias, options.equatorRadius, options.radialVariance, options.yJitter);
